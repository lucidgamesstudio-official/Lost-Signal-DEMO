...
_J'utilise un loading screen custom que je ne peux pas mettre pour des raisons de droits d'auteurs du studio_
...
Pour ajouter le loading screen à roblox studio vous avez **2** possibilités:

- Modifier le fichier `default.project.json` pour changer:
...
"ReplicatedFirst": {
  ...
  "Guis": {
	"$className": "Folder",
	"LoadingScreen": {
	  "$path": "asset/default-loading-screen.rbxmx"
	}
  }
},
```
